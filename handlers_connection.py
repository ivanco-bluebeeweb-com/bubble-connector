"""Connection lifecycle for Bubble Connector."""
...
@chat.function(
    "connect_bubble",
    "Connect Bubble account via credentials.",
    action_type="write",
    chain_callable=True,
    event="bubble-connector.connect_bubble",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_bubble(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Bubble Connector."""
    client = BubbleClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Bubble Account",
...
@chat.function(
    "list_connections",
    "List connected Bubble accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_bubble",
    "Disconnect Bubble account.",
    action_type="write",
    chain_callable=True,
    event="bubble-connector.disconnect_bubble",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_bubble(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
